...
<template>
  ...
  <div v-else class="inline-grid grid-flow-col gap-3">
    <UButton v-bind="$attrs" color="neutral" variant="subtle" :disabled="!supported" icon="i-lucide-folder-open" @click="openDir">
      {{ dirHandle ? 'Change' : 'Select' }} folder
    </UButton>
    <UBadge v-bind="badgeProps" :variant="dirHandle ? 'subtle' : 'solid'" :color="dirHandle ? `primary` : 'neutral'">
